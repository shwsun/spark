...
docker exec -u root -it spark-master rm -f /root/.ssh/known_hosts
...
# cat ./tmp-share/key_pub >> ./tmp-share/id_rsa.pub
docker cp spark-master:/root/.ssh/id_rsa.pub ./tmp-share/key_pub
cat ./tmp-share/key_pub >> ./tmp-share/id_rsa.pub
...
docker cp ./tmp-share/id_rsa.pub spark-master:/root/.ssh/authorized_keys
...
docker exec -u root -it spark-master chmod 0600 /root/.ssh/authorized_keys
...
docker exec -u root -it spark-master /etc/init.d/ssh stop;
docker exec -u root -it spark-master /etc/init.d/ssh start;